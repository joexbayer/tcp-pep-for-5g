...
sudo tc qdisc add dev $(ip route get $IP | grep -oP "(?<=dev )[^ ]+") parent 1:3 handle 3: fq_codel;
...
sudo tc qdisc add dev $IFACE parent 1:1 handle 10: netem delay 100ms;
sudo tc qdisc add dev $IFACE parent 10:1 fq_codel;
...
sudo tc qdisc add dev $IFACE parent 1:3 handle 30: netem delay 100ms;
...
sudo tc qdisc add dev $(ip route get $IP | grep -oP "(?<=dev )[^ ]+") parent 1:3 handle 3: fq_codel;
#                                                                                                      37500000
sudo tc qdisc add dev $(ip route get $IP | grep -oP "(?<=dev )[^ ]+") parent 1:3 handle 3: bfifo limit 37875000; # BDP  101ms
