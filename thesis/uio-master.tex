...
Figure \ref{fig:pep_bandwidth_delay} introduces delay to the topology. Without the PEP we would have a end to end connection with 15 Mbps and 110 ms delay (220 ms RTT). Assuming the connection between CD is highly fluctuating, between 15 Mbps and 60 Mbps, the time the TCP congestion controller has to react is 220 ms per RTT, the RTT of the connection between A and D. If, however, we split the connection at C, the time would be reduced to 20 ms, the RTT between C and D. This means the congestion controller, between C and D, can react 1000\% faster with a split connection. Thereby, better utilizing the fluctuating bandwidth.
...
This thesis will utilize a non-transparent PEP located at a base station. Mostly for the benefits of future adaptability and the client being aware of the PEP. Being located at the base station assures that the PEP will be on path...
