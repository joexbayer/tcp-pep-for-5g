...
An important aspect of Figure \ref{fig:graph1} is the big impact that the BFIFO queue has on the interactive traffic. The interactive traffic spikes up 240ms, which is the normal behavior with big network buffers and the reason buffers normally are kept small. While when using FQ CoDel the interactive flow is barely disrupted with and without PEP.\\
...
The next experiment will focus on the PEPs behavior in the context of realistic 5G mmwave traces.


\begin{figure}[h!] % htbp stand for "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.4]{../diagrams/witestlab/mmwave_cap.png}
  	\caption{mmWave capacity fluctuation}
  	\label{fig:graph3}
\end{figure}
text\cite{Srivastava_Fund_Panwar_2020}
