...
\end{figure}\footnote{Cant read "fluctuating" text on paper}
...
\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.55]{../diagrams/witestlab/throughput/all.png}
...
Continuing on the Mobility and Blockage scenario from the last section, we will evaluate how the buffer sizes on the PEP affect the performances. For this experiment we will run PEP configuration with ten different buffer sizes. Ranging from the default Linux (debian) socket buffers up to 128 megabytes.\\

The goal is to evaluate and confirm the buffers effect on the PEP, compared to the FQ-CoDel and BFIFO configuration.\\

\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.35]{../diagrams/witestlab/placeholder.png}
  	\caption{PLACEHOLDER.}
  	\label{fig:throughput_all}
\end{figure}
...
Focus on fairness vs throughput, bfifo is expected to have bad fairness, fqcodel good, and pep best, however, the mobb scenario might influence these-\\

Also talk about impact on interactive traffic, just to confirm its not much worse... 
