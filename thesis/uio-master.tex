...
\subsection{Experiment procedure}
To evaluate the PEP with the 5G traces we will change the topology to include a 100ms delay between the sender and the router. The link between the router and the receiver will get a 1ms delay for a more realistic scenario. 
...
The experiment procedure will be to send data from the sender to the receiver for 120 seconds. Measuring the throughput by seeing how much data was transferred in the 120 seconds. This procedure will be done three times, once for the PEP and twice for end-to-end. End-to-end will be evaluated with both FQ-CoDel and BFIFO, where the BFIFO queue will have a size of the BDP to show the effect how big queues in the network.\\

There will be ten simultaneous transfers to reduce the risk of a lucky or unlucky result.(Ref. to the cubic being unstable?) Additionally, we will have an end to end Ultra ping transfer to measure the latency disruptions for both the end-to-end and PEP file transfers.\\
...
\begin{itemize}
  ...
  \item \textbf{Step 3}: The ten senders start transferring data.
\end{itemize}
...
\subsection{Congestion Controller's affect on throughput}
When running an Iperf3 transfer from sender to receiver with a BDP sized BFIFO queue at the router, we observe an seemingly random results of throughput. The throughput should, with our configuration, be about 3Gbit/s, fully utilizing the bandwidth. However the results hover around 2.4Gbit/s to 2.8Gbits/s.\\

Looking at a congestion window graph of a cubic transfer we can see the reason:\\
CWND plot\\

The first interesting aspect of Figure ... is the slow cubic curve. It takes a long time to fully utilize the bandwidth. Additionally, looking at the relation to the BDP, we can see that the transfer was lucky the first time, and lucky the second. Resulting in a rather low throughput.\\

Looking at the double back-off after 70s in Figure ... we can see...\\

Which brings us to the conclusion that Cubic does not operate effectively in our scenarios with big bandwidths and huge buffers. Because of these results a single Cubic transfer is not representative of the achievable throughput as it will either be lucky and get a big throughput or unlucky with a low throughput.\\

Having multiple flows will reduce the luck aspect and give a more realistic representation of the throughput. Using Reno is not better... Reno graph.
...
The results in this experiment will focus on Ultra ping latency graphs to measure impact on interactive traffic and the completion times of the ten file transfers. To evaluate the PEP's efficiency, compared to a normal end to end transfer, we will look at the average, standard deviation and mean of the completion times.
