...
The static link scenario will represent a baseline of performance with little to no fluctuations. Based on this baseline we can compare the affect of the various types of fluctuations. 
...
Compared to our initial base case there seems to be a bigger impact on the interactive traffic with ten concurrent file transfers, however the delay's mean difference stays below 0,09ms at its worst, particularly the first 1000 packets. Another observation is that the interactive traffic has more drastic spikes when the PEP is utilized.\\
...
Table \ref{tab:sl_latency_statistics} highlights the differences between the end-to-end and PEP impact on interactive traffic. Most importantly, we can see that the PEP causes a higher standard deviation than the end-to-end traffic. This is reflected in the spikes of the Ultra ping graph in Figure \ref{fig:witestlab_static_delay}.\\
...
The mobility and blockage scenario is an overall benefit for the end-to-end traffic, as the high bandwidth phases allow the transfers to complete earlier. Looking at Figure \ref{fig:witestlab_mobb_fcts}, we can see that the highest completion time is about 104s. Table \ref{tab:mobb_fct_diff} shows that the mean completion time for end-to-end traffic decreased by approximately 11s.\\
...
To evaluate how effectively the PEP can utilize the fluctuating bandwidth, compared with end-to-end FQ-CoDel and BFIFO, we will look at the throughput using the mobility and blockage scenario. For this experiment we will continue with 19 parallel transfers from sender to receiver and the same traffic control configurations. The only difference is that the transfers will continue to send data for the entire 120 seconds duration.\\

\begin{figure}[h!] % htbp stand for "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.33]{../diagrams/witestlab/tput_bars_bdp.png}
\centering
\begin{tabularx}{\linewidth}{lCC}
\toprule
Scenario & Total MB/s & Total MBits/s\\
\midrule
PEP & 367.71 MB/s & 2941.70 Mbits/s \\
FQ-CoDel & 90.13 MB/s & 721.06 Mbits/s \\
BFIFO & 262.76 MB/s & 2102.13 Mbits/s \\
\bottomrule
\end{tabularx}
  	\caption{Throughput comparison of 19 flows for PEP and end to end transfers.}
  	\label{fig:witestlab_tput_bars}
\end{figure}

Figure \ref{fig:witestlab_tput_bars} shows the throughput of each of the 19 flows for both the PEP and end-to-end transfers. As we can see the PEP has a much higher throughput for all flows. If we sum up the throughput of all flows, then we can see that the PEP achieves a through put of 2941.70 Mbit/s, while the end-to-end FQ-CoDel transfers only manage 721.06 Mbit/s. Interestingly, the end-to-end transfer with BFIFO got a higher throughput of 2102.13 Mbit/s compared to end-to-end which used FQ-CoDel, this highlights the benefits of larger buffers.\\

Looking at the bars themselves, unfairness with bfifo, fair but low throughput fqcodel

\begin{figure}[h!] % htbp stand for "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.50]{../diagrams/witestlab/tput_ping_bdp.pdf}
  	\caption{Latency impact from throughput experiment.}
  	\label{fig:witestlab_tput_ping}
\end{figure}

Looking at the latency impact we will compare our PEP, end-to-end using FQ-CoDel and end-to-end using BFIFO with Ultra ping. Figure \ref{fig:witestlab_tput_ping} shows the Ultra ping graphs for each scenario. At its worst the end-to-end BFIFO case reaches a latency of 200ms, while both the PEP and end-to-end scenario show barely any impact at all. This highlights the major disadvantage of the larger buffers: the large impact on interactive traffic.\\

From the experiment we can see that the PEP manages to both increase the throughput by buffering the packets at the PEP, in this case router, and keep the impact on interactive traffic at a minimal by using FQ-CoDel. This would not be possible with traditional approaches as you are forced to choose between buffer size and delay, while the PEP offers both.
