...
\section{Proxy}
What is a proxy, use cases, benefits~\cite{nextgen_proxy_servers}.\\

Proxy servers play a big role in the modern internet, delivering benefits such as anonymity and increased performance. A common use case for a proxy is caching by keeping a copy of popular resources such as a websites. This reduces the latency of accessing the resource as long as the proxy is closer to the user than the original copy. Locality plays a important role in the total latency as any transmission will always be limited by the speed of light.\\
...
Linux is the most famous open source kernel freely available for anyone to use and modify. Various implemenation, full controll, benefits of open source, used by most servers (google, microsof etc). Good developer environment.

\subsection{Kernel Modules}
LKM (Loadable Kernel Modules), "program" running inside the Linux kernel.
Userspace vs Kernel. Reduce system call overhead. Speed, (bad portability)
