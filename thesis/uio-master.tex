...
\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	...
	\includegraphics[scale=0.70]{../diagrams/witestlab/throughput/throughput_latency_closeup_sl.pdf}
  	\caption{Closeup of the PEP and FQ-CoDel impact on interactive traffic for the static link configuration}
...
\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	...
	\includegraphics[scale=0.70]{../diagrams/witestlab/throughput/throughput_mobb.pdf}
  	\caption{Bar plot of throughout for each scenario with a mobility and blockage configuration}
...
Just over average throughput for the scenario for the PEP, still low std for PEP.\\

Looking at the impact on latency we see a similar result to the static link and short blockage. BFIFO has a large impact on the interactive traffic, especially in fluctuating scenarios, while both the PEP and FQ-CoDel show little to no impact. 

\subsection{Buffer size effect on throughput}

How the buffer sizes at the PEP affect the potential throughput for the mobility and blockage scenario-
...
The throughput experiment highlights the core ideas behind the PEP. Large buffers increase throughput, shown by the higher throughout when using a BFIFO buffer versus FQ-CoDel. However, the BFIFO queue also causes huge delays for any interactive traffic passing through. The PEP is able to utilize big buffers, while simultaneously utilizing FQ-CoDel as to not impact interactive traffic. Thereby better utilizing the fluctuating 5G bandwidth.
