...
The choice on transparency directly influences the deployment, adaptability, and overall interaction of the PEP with applications. We can tailor the PEP design to best suit the targeted application environment, ensuring optimized performance and adaptability. This section will discuss the design considerations, with a focus on Non-Transparent PEPs and their use in 5G networks.
...
This thesis will utilize a non-transparent PEP because of the benefits of future adaptability and the client being aware of the PEP itself. Because our PEP is designed for 5Gs fluctuating bandwidth, we can guarantee that the PEP is on the path. All traffic from a client will pass through the 5G base station? which means if we place the PEP at the base station, it will always be on path. \footnote{Are there more benefits?}\\

% This section will talk about using sockets for connection splitting, how the pep is addressed and the endpointed 
% conveyed to the PEP. 
\section{Connection handling}
All connections are handled by sockets because of their ability to be easily configured and because we intend to split the connections. This introduces challenges concerning how the PEP is addressed and utilized by a client. Additionally, the desired endpoint needs to be conveyed to the PEP. 
...
% This describes WHAT and why
The PEP is designed as a connection splitting PEP at the boundary of an unstable network with fluctuating bandwidth. By splitting the connection at this point we can buffer packets, usually closer to the user, and have them ready for high bandwidth phases. 
