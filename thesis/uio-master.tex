...
In Chapter 5 we will look at the evaluation of the performance enhancing proxy. The evaluation will consists of multiple experiments to confirm the effectiveness of the performance enhancing proxy itself. 
\chapter{Background}
...
The next experiment will focus on the PEPs behavior in the context of realistic 5G mmwave traces. About the witestlab experiments.~\cite{Srivastava_Fund_Panwar_2020}

\subsection{Traces}
The traces come from a ?

\subsection{Topology}
For this experiment we will use a topology similar to the previous experiment. We will continue our Sender, router and receiver topology, however, the traffic control configuration will change. Because of the high 5G bandwidth rates we will adjust the sender to router link to have 10Gbit/s bandwidth and 100ms delay.\\

The link between the router and the sender will start with a 3Gbit/s bandwidth and no delay. Additionally it will have a BFIFO queue with a max capacity of 7,5Mb which will create a delay of 20ms if the queue is fully filled.
...
For this experiment we will look at four different 5G scenarios: First a static link with little fluctuation in the bandwidth. The second scenario focuses on short blockages, the third on long blockages. And finally the last is the most realistic scenario featuring both mobility and blockages. 

Looking at the effect on raw delay.
...
Firstly we want to validate the experiments from the Witestlab.
