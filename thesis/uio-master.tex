...
\section{Summary}
In this chapter we have designed a high performance connection splitting PEP tailored for fluctuating bandwidth. We made the choice to keep it non-transparent to assure future adaptability and client configuration. Using sockets and their buffers we avoid bufferbloat and to not disrupt the interactive traffic. Finally, we use AQM and scheduling to avoid unnecessary delays on the outgoing links.\\
...
Normally a socket would first \verb|create|, \verb|connect| and then \verb|send| data using the \verb|send| system call. The creation of a socket is the same for both with and without TCP Fast Open. Which leaves us with connection as the main problem. The way we connect to a host using TCP Fast Open is by skipping \verb|connect| and instantly jump to \verb|sendto|. As we can see from the socket system calls, both \verb|connect| and \verb|sendto| take a \verb|struct sockaddr| as a parameter.\\
...
\end{verbatim}
\captionof{figure}{Socket Protocol Operations from Linux - net.h~\cite{linuxkernel_neth} }
\label{fig:proto_ops}
\end{minipage}
...
\begin{itemize}
  ...
  \item \textbf{Short Blockages}: Same as the static link scenario, however at a 15 second interval a human obstructs the path between the laptops.
...
\subsection{Topology}
For this experiment we will use a topology similar to the previous experiment. We will continue with our sender, router and receiver topology, however, the traffic control configuration will change. Because of the high 5G bandwidth we will adjust the sender to router link to have the maximum our testbed can offer, which is 10Gbit/s bandwidth.\\
...
\subsection{Configuration}
Since we are now dealing with high gigabit bandwidths, we need to configure and tune the hosts to be able to fully utilize these bandwidths. Most importantly the default net read and write buffers need to be adjusted.\footnote{Show the behaviour if we do not?}\\

The the core net buffers \verb|net.core.rmem_max| and \verb|net.core.wmem_max| will be adjusted to approximately 2 Gigabyte. Also \verb|net.ipv4.tcp_rmem| and \verb|net.ipv4.tcp_wmem| will be set to the same 2 Gigabyte size. This will assure that neither the sender or receiver is limited by the socket buffers.
...
Following each step of the original experiment, we configure the link between the router and the receiver to have a 3Gbit/s bandwidth and no configured delay. Additionally, it will have a BFIFO queue with a max capacity of 7,5Mb which will create a delay of 20ms if the queue is fully filled. This configuration is visualized in Figure\ref{fig:tc_5g_bfifo} \\
...
Figure \ref{fig:witestlab_results} shows the latency results from the Witestlab. We will focus on their TCP Cubic results as it is the congestion controller we use for our experiments \footnote{Linxu kernel version, many details as possible, hardware}. When using Cubic, Witestlab experience delays up to 150ms in the worst cases for long blockage.\\
...
The end-to-end scenario has two cases, once with FQ-CoDel and once with BFIFO. FQ-CoDel will reduce the amount of impact on the interactive, while BFIFO will allow the senders to buffer a lot of data at the router. The BFIFO buffer will be configured with a queue size of the BDP based on the static scenario. The total end to end RTT is 101ms and the bottleneck bandwidth is 3gbit/s. Based on the calculation from Figure \ref{fig:bdp_calculation} the BDP will is 37,875,000 bytes. This will allow for a lot of data to be buffered at the router, which will negatively impact the interactive traffic, but increase the throughput.\\
...
For the PEP case we will change the configuration to assert that no delay is added from the sender. We will achieve this by adding FQ-CoDel at the senders interface and configure the 100ms delay on the routers interface towards the sender. Because the PEP essentially will be a split connection at the router we must be sure that interactive traffic is not already delayed before it reaches the PEP.\\

Mention PEP buffer sizes?
...
While running a throughput test from sender to receiver with a BDP sized BFIFO queue at the router, we observe seemingly random results of throughput. The throughput should theoretically, with our configuration, be about 3Gbit/s: fully utilizing the bandwidth. However the results hover around 2.1Gbit/s to 2.5Gbits/s. To understand this we can look at the congestion window of a cubic transfer.\\
...
The first interesting aspect of Figure \ref{fig:cubiccwnd} is the slow cubic curve. It takes a long time to fully utilize the bandwidth. Additionally, looking at the relation to the BDP, we can see that the transfer was lucky the first time starting just under the BDP, and unlucky the second were it starts way below the BDP. Resulting in a rather low throughput.\\

Looking at the double back-off after 70 seconds in Figure \ref{fig:cubiccwnd} we can see that the next congestion avoidance? phase starts low and does not reach full throughput again before the experiment ended.Which brings us to the conclusion that Cubic does not operate effectively in our scenarios with big bandwidths and huge buffers. Because of these results a single Cubic transfer is not representative of the achievable throughput as it will either be lucky and get a big throughput or unlucky with a low throughput.\\

Looking at Reno we see the same kind of behavior, where the big BFIFO queue and high bandwidth results in low throughput because of the congestion avoidance phase. Looking at Figure \ref{fig:renocwnd} we can see an unlucky congestion avoidance start which leads to low throughput.
...
% The experiments will consist of comparing, end to end fqcodel, bfifo and the PEP, with the hypohethis being that BFIFO will have good through put and fqcodel not. 
...
The static link scenario will represent a baseline of performance with little to no fluctuations. Based on this baseline we can compare the affect of the various types of fluctuations. The static link configuration has a average throughput of 2905.61 Mbit/s and a standard deviation of 171.74 Mbit/s.\\

\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	\centering
...
The PEP has the highest throughput of 2737.83 Mbit/s, almost reaching the average throughput of the scenario. The PEP throughput highlights the benefit of both large buffers and a split connection.\\
...
\hline
FQ-CoDel & 1998.12 Mbit/s & 42.10 Mbit/s & 1907.12 Mbit/s & 2059.27 Mbit/s \\
BFIFO    & 2382.00 Mbit/s & 63.76 Mbit/s & 2289.70 Mbit/s & 2453.32 Mbit/s \\
PEP      & 2737.83 Mbit/s & 11.67 Mbit/s & 2704.81 Mbit/s & 2742.91 Mbit/s \\
\hline
\end{tabular}
...
Figure \ref{fig:throughput_latency_sl} shows the impact on the interactive each configuration has. 
Looking at the BFIFO configuration we get a huge impact on the interactive traffic because of the large buffer. This shows the cost of the higher throughput that BFIFO achieves over FQ-CoDel. The base RTT of our configuration is 101ms, the BFIFO reaches peaks above 200ms and a average of 146.83ms with a 26.59ms standard deviation.\\
...
The short blockage is an important scenario to evaluate as it will show the impact only small and short fluctuations will have on the throughput, especially for FQ-CoDel. The average throughput of the short blockage configuration is 2726.27 Mbit/s with a 517.02 Mbit/s standard deviation.\\
...
From Figure \ref{fig:throughput_sb} we can see an overall decrease of throughput, which is expected based on the average throughput of the scenario. However, especially FQ-CoDel has a big decrease in throughput from a 1998.12 Mbit/s average throughput in static link down to 1017.17 Mbit/s in the short blockage scenario. In this scenario the benefit of big buffers becomes especially apparent, the low buffer sizes of FQ-CoDel with fluctuations forces FQ-CoDel to drop packets, which is devastating for the congestion controller.\\
...
\hline
 & \textbf{mean} & \textbf{std} & \textbf{min} & \textbf{max} \\ 
\hline
FQ-CoDel & 1017.17 Mbit/s & 150.29 Mbit/s & 690.74 Mbit/s & 1105.09 Mbit/s\\
BFIFO    & 2090.83 Mbit/s & 56.05 Mbit/s & 2006.43 Mbit/s & 2207.30 Mbit/s\\
PEP      & 2530.90 Mbit/s & 1.33 Mbit/s & 2528.78 Mbit/s & 2533.52 Mbit/s \\
\hline
\end{tabular}
...
Table \ref{tab:throughput_sl_table} compares the throughput of each configuration, and shows the drastic drop in throughput for FQ-CoDel. Additionally, FQ-CoDel has a higher standard deviation compared to the static link scenario. Both BFIFO and the PEP are able to buffer more packets, which makes them more tolerate to fluctuations.\\
...
Looking at Figure \ref{fig:throughput_lb} we can see a positive affect with the FQ-CoDel configuration. For FQ-CoDel the rapid fluctuations are the worst case scenarios, because it is unable to react fast enough or buffer enough packets to not drop packets. Table \ref{tab:throughput_lb_table} we can see that the FQ-Codel throughput increased by 159.80 Mbit/s while the standard deviation decreased by 102.74 Mbit/s compared to the short blockage scenario.
...
\hline
 & \textbf{mean} & \textbf{std} & \textbf{min} & \textbf{max} \\ 
\hline
FQ-CoDel & 1176.97 Mbit/s      & 47.55 Mbit/s       & 1121.00 Mbit/s     & 1253.09 Mbit/s     \\
BFIFO    & 1852.70 Mbit/s      & 58.61 Mbit/s       & 1751.37 Mbit/s     & 1956.78 Mbit/s     \\
PEP      & 2368.77 Mbit/s      & 5.36  Mbit/s       & 2354.20 Mbit/s     & 2372.09 Mbit/s     \\
\hline
\end{tabular}
\caption{Statistics of the throughout for the short blockage scenario.}
...
The mobility and blockage scenario will be the most interesting scenario for our experiment, as it captures the volatile nature of 5G mmWave networks. The high fluctuations and both increase and decrease in bandwidth from the original static 3 Gbit/s. Because of these bandwidth peeks along side the drops we get an average throughput of 2962.61 Mbit/s which is the highest average throughput of all scenarios. This comes at the cost of the highest standard deviation of almost 1Gbit/s, more exactly 826.45 Mbit/s.\\

\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.70]{../diagrams/witestlab/throughput/throughput_mobb.pdf}
  	\caption{Bar plot of throughout for each scenario with a mobility and blockage scenario}
  	\label{fig:throughput_mobb}
\end{figure}

Figure \ref{fig:throughput_mobb} shows the throughput of each configuration, most notably the PEP has a big increase from the previous scenario. The PEP has a average throughput of 2992.35 Mbit/s which is over the average of the scenario. This result clearly shows the benefit of the PEP, especially in a highly fluctuating environment as in this scenario. The PEP is able to utilize the high bandwidth phases by buffering the most data and reacting faster than any other configuration.\\

The BFIFO configuration also has a increase of 435.05 Mbit/s from the previous scenario, in contrast to FQ-CoDel which had an decrease in throughput compared to the previous scenario. This highlights the importance of larger buffers in highly fluctuating environments. However, the BFIFO configuration suffers from a much higher standard deviation of 283.13 Mbit/s and an approximately difference of 1Gbit/s between the Min and Max throughput results.\\
...
\hline
 & \textbf{mean} & \textbf{std} & \textbf{min} & \textbf{max} \\ 
\hline
FQ-CoDel & 1074.42 Mbit/s      & 96.78 Mbit/s       & 951.73 Mbit/s      & 1185.74 Mbit/s     \\
BFIFO    & 2287.75 Mbit/s      & 283.13 Mbit/s      & 1653.52 Mbit/s     & 2694.71 Mbit/s     \\
PEP      & 2992.35 Mbit/s      & 10.07 Mbit/s       & 2975.52 Mbit/s     & 3002.52 Mbit/s     \\
\hline
\end{tabular}
...
\section{Buffer size effect on throughput}
Continuing on the Mobility and Blockage scenario from the last section, we will evaluate how the buffer sizes on the PEP affect the performances. For this experiment we will run PEP configuration with ten different buffer sizes. The evaluation will consist of ten steps, ranging from the default Linux (debian) socket buffers up to 128 megabytes.\\

The Linux default socket buffers sizes start at 2 megabyte for net core read and write buffers. The TCP read buffers are at 6 megabyte, and the write buffers at 4 megabyte. The default buffers will be increased at a 1/10 interval untill they reach 128 megabyte. Each step, we will run one flow using the PEP for the entire 120 seconds. For a consistent result we will run each step ten times and take the average as the final result. The goal is to evaluate and confirm the buffers effect on the PEP, compared to the FQ-CoDel and BFIFO configuration. \\

\begin{figure}[!h!] % htbp stand f	or "here", "top", "bottom", "page"
	\centering
	\includegraphics[scale=0.55]{../diagrams/witestlab/throughput/buffersizes.pdf}
  	\caption{Throughput for the PEP based on buffer sizes in ten steps.}
  	\label{fig:throughput_all}
\end{figure}

As we can see from Figure \ref{fig:throughput_all}, the default buffer sizes preform the worst, improving with each step. From the default to step 7 the throughput increases with bigger buffer sizes. This highlights the importance of the buffer sizes for the PEP. After step 7, approximately 85.40 MB buffer size, the throughput reaches its limit based on the scenario.
...
\begin{figure}[ht]
  \centering
  \subfloat[Throughput for 10 flows with BFIFO]{
  	\includegraphics[width=0.45\textwidth]{
  		../diagrams/witestlab/throughput/10flows/BFIFO.pdf
  	}
  	\label{fig:subfig1}}
  \hfill
  \subfloat[Throughput for 10 flows with FQ-CoDel]{
  	\includegraphics[width=0.45\textwidth]{
  		../diagrams/witestlab/throughput/10flows/FQ-CoDel.pdf
  	}
  	\label{fig:subfig2}}
  \hfill
  \subfloat[Throughput for 10 flows with PEP]{
  	\includegraphics[width=0.45\textwidth]{
  		../diagrams/witestlab/throughput/10flows/PEP.pdf
  	}
  	\label{fig:subfig3}}
  \caption{Overall caption for the set of subfigures}
  \label{fig:completefig}
\end{figure}

Figure \ref{fig:completefig} shows the ten flows for each of the three configurations. The main takeaways are the loss in total throughput for BFIFO and FQ-CoDel, having multiple flows compete with the heavy fluctuations lead to less bandwidth utilization. The PEP only has minimal throughput loss, even with multiple flows.\\

Another interesting is the aspect of fairness. Using BFIFO we can see a clear unfairness between the 10 flows. One of the beneficial side effects of FQ-CoDel is its inherent fairness, as we can see in Figure \ref{fig:completefig} (b). Because the PEP also uses FQ-CoDel, we get the same fairness, but without the loss in throughput.\\
