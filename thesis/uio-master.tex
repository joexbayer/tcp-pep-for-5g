...
\section{TLV Library}
Regarding the addressing and endpoint selection of the PEP by the client, a custom shared library is a good choice.
\footnote{Rewrite this.} The goal is to keep the client code as simple and close to its original state as possible, but still be able to communicate the endpoint, potential options and meta-data to the PEP. Additionally, to be able to send this information we use TCP Fast Open which has a similar connection setup as default sockets do.\\
