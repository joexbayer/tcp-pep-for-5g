...
\section{Socker buffer size effect on throughput}
...
Another interesting aspect is the flow fairness\footnote{Talk about jains fairness}, as we can see that with BFIFO there is a clear unfairness between the 10 flows. One of the beneficial side effects of FQ-CoDel is its inherent fairness, as we can see in Figure \ref{fig:completefig} (b). Because the PEP also uses FQ-CoDel, we get the same fairness, but without the loss in throughput.\\
