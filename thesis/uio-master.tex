...
Important aspects of Figure \ref{fig:graph1} is the big impact that the BFIFO queue has on the interactive traffic. The interactive traffic spikes up 240m, which is the normal behavior with big network buffers and the reason normally buffers are kept small. While when using FQ CoDel the interactive flow is barely disrupted with and without PEP.\\
